...
execSync(`npx wrangler d1 migrations apply maestro-db ${flag}`, {
  cwd: apiDir,
  stdio: "inherit",
});
...
execSync(
  `npx wrangler d1 execute maestro-db ${flag} --file=seed/seed-data.sql`,
...
const result = execSync(
  `npx wrangler d1 execute maestro-db ${flag} --json --command="SELECT id, label, submitted_by FROM templates"`,
